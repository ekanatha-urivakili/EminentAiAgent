...
- **Terminal Observer:** Proactive "💡 Fix with LocalForge" integration for build/test errors in the VS Code extension.
...
flowchart TB
    ...
    WEB -->|REST + SSE/WebSocket| API[LocalForge Backend\nASP.NET Core 8 Minimal API]
    VSC -->|REST + SSE| API
...
localforge/
├─ src/
│  ├─ LocalForge.Domain/            # entities, value objects, no deps
...
│  ├─ LocalForge.Application/       # use cases, interfaces
...
│  └─ LocalForge.Api/               # Minimal API + SSE endpoints
...
sequenceDiagram
    ...
    participant API as LocalForge Backend
...
**SMTP configuration** (`appsettings.json` → `LocalForge:Smtp`):

| Key | Default | Purpose |
|---|---|---|
| `Host` | `127.0.0.1` | SMTP host (Mailpit) |
| `Port` | `1025` | Mailpit SMTP port |
| `FromAddress` | `noreply@localforge.local` | Envelope from |
...
| Data at rest | SQLite optional SQLCipher; uploads under `~/Library/Application Support/LocalForge` |
...
5. **Terminal Observer** — Extension monitors terminal output for errors (e.g., build/test failures) and offers proactive "💡 Fix with LocalForge" suggestions.

Architecture decision: the extension talks to **your LocalForge backend, not Ollama directly**. You get history, RAG, MCP, and policy for free, and one protocol to maintain. (Direct-to-Ollama fallback is a settings toggle for chat-only use.)
...
flowchart LR
    ...
    CL -->|127.0.0.1:5210| LF[LocalForge Backend]
    LF --> OL[(Ollama)]
...
yo code   # → New Extension (TypeScript), bundler: esbuild, name: localforge-copilot
cd localforge-copilot && npm i
```
...
```jsonc
{
  "name": "localforge-copilot",
  "displayName": "LocalForge Copilot (Ollama)",
  "engines": { "vscode": "^1.95.0" },
  "main": "./dist/extension.js",
  "activationEvents": ["onStartupFinished"],
  "contributes": {
    "viewsContainers": { "activitybar": [
      { "id": "localforge", "title": "LocalForge", "icon": "media/icon.svg" } ] },
    "views": { "localforge": [
      { "type": "webview", "id": "localforge.chat", "name": "Chat" } ] },
    "commands": [
      { "command": "localforge.explain",   "title": "LocalForge: Explain Selection" },
      { "command": "localforge.fix",       "title": "LocalForge: Fix Selection" },
      { "command": "localforge.refactor",  "title": "LocalForge: Refactor Selection" },
      { "command": "localforge.tests",     "title": "LocalForge: Generate Tests" },
      { "command": "localforge.agentEdit", "title": "LocalForge: Agent Edit (multi-file)" },
      { "command": "localforge.pickModel", "title": "LocalForge: Switch Model" }
    ],
    "menus": { "editor/context": [
      { "command": "localforge.explain", "when": "editorHasSelection", "group": "localforge@1" },
      { "command": "localforge.fix",     "when": "editorHasSelection", "group": "localforge@2" } ] },
    "keybindings": [
      { "command": "localforge.explain", "key": "ctrl+alt+e", "mac": "cmd+alt+e" } ],
    "configuration": { "title": "LocalForge", "properties": {
      "localforge.backendUrl":   { "type": "string", "default": "http://127.0.0.1:5210" },
      "localforge.ollamaUrl":    { "type": "string", "default": "http://127.0.0.1:11434",
                                   "description": "Fallback when backend is offline" },
      "localforge.chatModel":    { "type": "string", "default": "qwen2.5-coder:7b" },
      "localforge.fimModel":     { "type": "string", "default": "qwen2.5-coder:1.5b-base" },
      "localforge.inlineCompletions": { "type": "boolean", "default": true },
      "localforge.maxPrefixTokens":   { "type": "number", "default": 2000 }
    }}
  }
}
```
    ...
    vscode.commands.registerCommand("localforge.agentEdit", async () => {
      ...
      for await (const ev of api.streamRun(run.id)) {
        if (ev.type === "file_edit_proposed") {
          ...
          const proposed = orig.with({ scheme: "localforge-proposed" });   // TextDocumentContentProvider
...
# Package → .vsix
vsce package          # produces localforge-copilot-0.1.0.vsix

# Install locally
code --install-extension localforge-copilot-0.1.0.vsix
...
# Backend
dotnet new sln -n LocalForge && cd src
dotnet new classlib -n LocalForge.Domain
dotnet new classlib -n LocalForge.Application
dotnet new classlib -n LocalForge.Infrastructure
dotnet new web      -n LocalForge.Api
dotnet add LocalForge.Infrastructure package ModelContextProtocol --prerelease
dotnet add LocalForge.Infrastructure package Microsoft.EntityFrameworkCore.Sqlite
...
npx -y @modelcontextprotocol/server-filesystem ~/projects/localforge
```
