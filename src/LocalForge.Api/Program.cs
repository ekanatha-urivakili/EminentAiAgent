...
using LocalForge.Application.Planning;
using LocalForge.Domain;
using LocalForge.Infrastructure.Mcp;
using LocalForge.Infrastructure.Ollama;
using LocalForge.Infrastructure.Persistence;
using LocalForge.Infrastructure.Security;
using LocalForge.Infrastructure.Tools;
...
// ---------------------------------------------------------------------------
var configuredToken = builder.Configuration["LocalForge:ApiToken"];
var apiToken = !string.IsNullOrWhiteSpace(configuredToken)
    ? configuredToken
    : Environment.GetEnvironmentVariable("LOCALFORGE_API_TOKEN");
...
if (!isLoopback && string.IsNullOrWhiteSpace(apiToken))
    throw new InvalidOperationException(
        "Refusing to bind a non-loopback address without LOCALFORGE_API_TOKEN set (see architecture N4).");
builder.WebHost.UseUrls(urls);

// ---------------------------------------------------------------------------
// Services
// ---------------------------------------------------------------------------
builder.Services.AddDbContextFactory<LocalForgeDbContext>(options =>
    options.UseSqlite(builder.Configuration.GetConnectionString("Default") ?? "Data Source=localforge.db"));

builder.Services.AddHttpClient<IOllamaClient, OllamaClient>(client =>
{
    client.BaseAddress = new Uri(builder.Configuration["LocalForge:OllamaUrl"] ?? "http://127.0.0.1:11434");
...
var allowedOrigins = builder.Configuration.GetSection("LocalForge:AllowedOrigins").Get<string[]>()
...
{
    var smtp = config.GetSection("LocalForge:Smtp");
    var host = smtp["Host"] ?? "127.0.0.1";
    var port = int.Parse(smtp["Port"] ?? "1025");
    var from = smtp["FromAddress"] ?? "noreply@localforge.local";
    var fromName = smtp["FromName"] ?? "LocalForge";
...
#pragma warning disable CS0618 // SmtpClient is deprecated but works fine with Mailpit on localhost
    ...
    {
        Subject = "Reset your LocalForge password",
        Body = $"""
            Someone requested a password reset for your LocalForge admin account.
...
// Admin auth
// ---------------------------------------------------------------------------
app.MapGet("/api/auth/me", async (HttpRequest request, IDbContextFactory<LocalForgeDbContext> dbFactory, CancellationToken ct) =>
{
...
app.MapPost("/api/auth/login", async ([FromBody] LoginRequest request, IDbContextFactory<LocalForgeDbContext> dbFactory, CancellationToken ct) =>
{
    await using var db = await dbFactory.CreateDbContextAsync(ct);
    var email = request.Email.Trim().ToLowerInvariant();
...
app.MapPost("/api/auth/forgot-password", async ([FromBody] ForgotPasswordRequest request, IDbContextFactory<LocalForgeDbContext> dbFactory, IConfiguration config, CancellationToken ct) =>
{
...
app.MapPost("/api/auth/reset-password", async ([FromBody] ResetPasswordRequest request, IDbContextFactory<LocalForgeDbContext> dbFactory, CancellationToken ct) =>
{
...
app.MapPost("/api/connectors", async ([FromBody] ConnectorRequest request, IDbContextFactory<LocalForgeDbContext> dbFactory, CancellationToken ct) =>
{
...
app.MapDelete("/api/connectors/{id:guid}", async (Guid id, IDbContextFactory<LocalForgeDbContext> dbFactory, CancellationToken ct) =>
{
    await using var db = await dbFactory.CreateDbContextAsync(ct);
...
{
    var dbFactory = scope.ServiceProvider.GetRequiredService<IDbContextFactory<LocalForgeDbContext>>();
