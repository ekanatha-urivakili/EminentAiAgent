...
function CmdLine({ cmd }: { cmd: string }) {
  return (
    <div className="flex items-center justify-between gap-2 rounded-md bg-muted/70 border border-border px-2.5 py-1.5 font-mono text-[11px]">
  ...
  return (
    <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 overflow-hidden">
      <button onClick={onToggle} className="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-amber-500/10 transition-colors">
        ...
        <ChevronDownIcon size={14} className={cn('text-amber-500 transition-transform flex-shrink-0 ml-2', open && 'rotate-180')} />
      </button>
      {open && (
        <div className="px-4 pb-4 space-y-4 border-t border-amber-500/20">
          ...
          <div className="space-y-1.5">
            <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide">macOS — Homebrew (easiest)</p>
            ...
            <p className="text-[11px] text-muted-foreground">If the model file is missing, download it first:</p>
            <CmdLine cmd="whisper-cpp-download-ggml-model base.en" />
          </div>

          <div className="space-y-1.5">
            <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wide">Linux / macOS — build from source</p>
          ...
          <div className="space-y-1.5">
            ...
            <a href="https://github.com/ggerganov/whisper.cpp/releases" target="_blank" rel="noopener noreferrer"
              ...
              <ExternalLink size={11} /> Download pre-built binary from GitHub releases
            </a>
            <CmdLine cmd="whisper-server.exe -m models/ggml-base.en.bin -p 8082" />
          </div>

          <div className="rounded-md bg-muted/60 border border-border px-3 py-2 text-[11px] text-muted-foreground space-y-1">
      ...
      <div className="max-w-3xl w-full space-y-2">
        ...
        <div className="relative rounded-3xl border border-input bg-card shadow-lg shadow-black/5 focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary/40 transition-all flex flex-col">
            ...
            <div className="px-5 pt-3 pb-1 flex flex-wrap gap-2">
              {attachments.map((a, i) => (
                <div key={`${a.name}-${i}`} className="relative group">
                  ...
                  <button onClick={() => removeAttachment(i)} className="absolute -right-2 -top-2 bg-background border border-border rounded-full p-1 shadow-sm opacity-90 hover:opacity-100"><X size={13} /></button>
                </div>
              ))}
              {textAttachments.map((a, i) => (
                <div key={`${a.name}-${i}`} className="flex items-center gap-1.5 bg-muted text-foreground text-xs px-2.5 py-1.5 rounded-lg border border-border">
                  <span className="font-medium max-w-[120px] truncate">{a.name}</span>
                  <button onClick={() => removeTextAttachment(i)} className="text-muted-foreground hover:text-foreground transition-colors"><X size={14} /></button>
                </div>
              ))}
            </div>
          )}
          ...
          <textarea
            ...
            rows={1}
            className={cn(
              'w-full bg-transparent rounded-3xl pl-4 sm:pl-5 pr-14 pb-24 sm:pb-12 focus:outline-none resize-none max-h-48 overflow-y-auto custom-scrollbar text-sm md:text-[15px]',
              attachments.length > 0 ? 'pt-2' : 'pt-4',
            )}
            style={{ minHeight: '112px' }}
          />

          <div className="absolute left-3 right-14 bottom-2.5 z-10 flex flex-wrap items-center gap-1.5 sm:gap-2">
            {/* "+" attachment */}
            <div ref={plusMenuRef} className="relative">
              <button
                onClick={() => setPlusMenuOpen((v) => !v)}
                className="flex items-center justify-center w-8 h-8 rounded-full hover:bg-muted text-muted-foreground hover:text-foreground transition-colors border border-border bg-background/60"
                title="Attach"
              >
                <Plus size={16} />
              </button>
                ...
                {plusMenuOpen && (
                  ...
                  >
                    <button
                      ...
                      className="flex items-center gap-3 w-full px-3.5 py-2.5 text-sm text-left hover:bg-muted transition-colors"
                    >
                    ...
                    {recentFiles.length > 0 && (
                      <>
                        ...
                        <div className="px-3.5 py-1 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                        ...
                        {recentFiles.map((f) => (
                          <button
                            ...
                            className="flex items-center gap-2.5 w-full px-3.5 py-2 text-sm text-left hover:bg-muted transition-colors"
                          >
                            <span className="text-muted-foreground flex-shrink-0">{fileTypeIcon(f.type)}</span>
                            <span className="truncate flex-1">{f.name}</span>
                            <span className="text-[10px] text-muted-foreground flex-shrink-0">{new Date(f.lastUsed).toLocaleDateString()}</span>
                          </button>
                        ))}
                      </>
                    )}
            ...
            <div ref={modeMenuRef} className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full hover:bg-muted text-xs font-medium transition-colors border border-border bg-background/60"
              >
                <ModeIcon size={13} className={modeMeta[mode].color} />
                <span>{mode}</span>
                <ChevronDown size={12} className={cn('text-muted-foreground transition-transform', modeMenuOpen && 'rotate-180')} />
              </button>
              <AnimatePresence>
                {modeMenuOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 6, scale: 0.96 }}
                    transition={{ duration: 0.13 }}
                    className="absolute bottom-full left-0 mb-2 w-56 bg-popover text-popover-foreground border border-border rounded-xl shadow-xl overflow-hidden py-1.5"
                  >
                    {(Object.keys(modeMeta) as Mode[]).map((m) => {
                      ...
                      return (
                        <button
                          ...
                          className="flex items-center gap-3 w-full px-3.5 py-2.5 text-sm text-left hover:bg-muted transition-colors relative"
                        >
                          {mode === m && <motion.div layoutId="activeMode" className="absolute left-0 top-1 bottom-1 w-0.5 rounded-full bg-primary" />}
                          <Icon size={15} className={modeMeta[m].color} />
                          <div className="flex flex-col">
                            <span className="font-medium leading-none mb-1">{m}</span>
                            <span className="text-[10px] text-muted-foreground leading-none">{modeMeta[m].hint}</span>
                          </div>
                        </button>
                      );
                    })}
            ...
            <button
              ...
              className={cn(
                'flex items-center gap-1 px-2 py-1 rounded-full text-[11px] font-medium transition-colors border',
                voiceModeEnabled
            ...
            >
              {voiceModeEnabled ? <Volume2 size={12} /> : <VolumeX size={12} />}
              <span className="hidden sm:inline">Voice</span>
            </button>
          </div>

          {busy ? (
            <button onClick={stop} className="absolute right-3 bottom-3 flex items-center justify-center w-9 h-9 rounded-full bg-foreground text-background shadow-md hover:scale-105 transition-all" title="Stop">
              <Square size={14} className="fill-current" />
            </button>
          ) : (
            <button
              onClick={submit}
              disabled={(!input.trim() && attachments.length === 0 && textAttachments.length === 0) || !selectedModel}
              className={cn(
                'absolute right-3 bottom-3 flex items-center justify-center w-9 h-9 rounded-full transition-all',
                (input.trim() || attachments.length > 0 || textAttachments.length > 0) && selectedModel
                  ? 'bg-foreground text-background shadow-md hover:scale-105'
        ...
        <div className="text-center text-[11px] text-muted-foreground tracking-wide opacity-70">
